...
- Brand applications in both regions and Tashkent Spectre applications are plain messages: no operator buttons, comment relay, or archiving. Startup removes old stored brand/Spectre keyboards without deleting their messages. Operator controls are only for driver applications.
